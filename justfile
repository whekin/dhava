...
stack-up:
    POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-nakvali-local}" API_ACCESS_KEY="${API_ACCESS_KEY:-nakvali-local}" PUBLIC_BASE_URL="${PUBLIC_BASE_URL:-http://127.0.0.1:8080}" docker compose --project-directory . -f deploy/docker-compose.yml -f deploy/docker-compose.local.yml up --build

# Stop the local stack without deleting its database volume.
stack-down:
    POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-nakvali-local}" API_ACCESS_KEY="${API_ACCESS_KEY:-nakvali-local}" PUBLIC_BASE_URL="${PUBLIC_BASE_URL:-http://127.0.0.1:8080}" docker compose --project-directory . -f deploy/docker-compose.yml -f deploy/docker-compose.local.yml down
