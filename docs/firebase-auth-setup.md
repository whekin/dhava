...
6. Under **Project settings → Service accounts**, create credentials for the Go API.
   ...
   `FIREBASE_SERVICE_ACCOUNT_JSON`, paste the JSON compacted with `jq -c .`, enable
   **Literal** and **Locked**, disable **Multiline**, keep it **Runtime-only**, and
   disable Build Variable. Compose consumes this value as a runtime secret and mounts
   it read-only; never commit or paste the private key into Gradle properties, Compose
   files, logs, or chat.
...
Set `FIREBASE_PROJECT_ID=nakvali-app` and the locked literal runtime variable
`FIREBASE_SERVICE_ACCOUNT_JSON` in Coolify, then redeploy. Compose mounts the latter at
`/run/secrets/firebase-service-account.json`; backend configuration passes that fixed
path directly to the Firebase Admin SDK. Do not create `GOOGLE_APPLICATION_CREDENTIALS`
in Coolify. Choose an account from Profile on a test build: the expected final state is
`Synced`, and Postgres should contain one
