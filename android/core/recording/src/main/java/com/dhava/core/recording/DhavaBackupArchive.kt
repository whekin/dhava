...
class DhavaBackupException(message: String, cause: Throwable? = null) :
 ...
 */
internal class DhavaBackupArchive(
        ...
        try {
            ZipInputStream(input.buffered()).use { zip ->
                ...
                while (true) {
                    ...
                    if (entry.isDirectory) throw DhavaBackupException("Backup contains an unexpected directory")
                    val manifestEntry = expected[entry.name]
                        ?: throw DhavaBackupException("Backup contains an unexpected file: ${entry.name}")
                    if (!seen.add(entry.name)) {
                        throw DhavaBackupException("Backup contains the same file twice: ${entry.name}")
                    }
                    ...
                    target.outputStream().buffered().use { output ->
                        ...
                        while (true) {
                            ...
                            if (written > manifestEntry.size) {
                                throw DhavaBackupException("Backup file is larger than declared: ${entry.name}")
                            }
                    ...
                    if (written != manifestEntry.size || digest.hexDigest() != manifestEntry.sha256) {
                        throw DhavaBackupException("Backup checksum failed: ${entry.name}")
                    }
                    zip.closeEntry()
                }
                val missing = expected.keys - seen
                if (missing.isNotEmpty()) {
                    throw DhavaBackupException("Backup is incomplete: ${missing.first()}")
                }
    ...
    private fun readManifest(zip: ZipInputStream): BackupManifest {
        ...
        while (true) {
            ...
            if (total > MAX_MANIFEST_BYTES) {
                throw DhavaBackupException("Backup manifest is too large")
            }
        ...
        return runCatching {
            ...
            throw DhavaBackupException("Backup manifest is invalid", error)
    ...
    private fun validateManifest(manifest: BackupManifest, stagingDirectory: File) {
        ...
        if (total > usable - reserve) {
            throw DhavaBackupException("Not enough free space to verify this backup")
        }
    }

    private fun validateManifestShape(manifest: BackupManifest) {
        if (manifest.formatVersion != FORMAT_VERSION) {
            throw DhavaBackupException("Backup format ${manifest.formatVersion} is not supported")
        }
        if (manifest.entries.size > MAX_ENTRY_COUNT) {
            throw DhavaBackupException("Backup contains too many files")
        }
        if (manifest.recordingCount < 0 || manifest.segmentCount < 0 || manifest.importedTraceCount < 0) {
            throw DhavaBackupException("Backup manifest contains an invalid item count")
        }
        if (manifest.entries.map(BackupManifestEntry::path).toSet().size != manifest.entries.size) {
            throw DhavaBackupException("Backup manifest contains duplicate files")
        }
        manifest.entries.forEach { entry ->
            if (!allowedPath(entry.path)) throw DhavaBackupException("Backup path is not allowed: ${entry.path}")
            if (entry.size < 0L) throw DhavaBackupException("Backup contains an invalid file size")
            if (!SHA256_PATTERN.matches(entry.sha256)) {
                throw DhavaBackupException("Backup contains an invalid checksum")
            }
        }
        manifest.totalBytes()
    }
    ...
    private fun writeStoredEntry(
        ...
        if (size != prepared.size || crc.value != prepared.crc32 || digest.hexDigest() != prepared.sha256) {
            throw DhavaBackupException("Source changed while backup was being created: $path")
        }
        zip.closeEntry()
    }
    ...
    private fun decodeRecordings(bytes: ByteArray): List<LocalRecording> = runCatching {
        ...
        throw DhavaBackupException("Recording index could not be backed up", error)
    }
    ...
    private fun BackupManifest.totalBytes(): Long {
        ...
        entries.forEach { entry ->
            total = try {
                ...
                throw DhavaBackupException("Backup size is invalid")
            }
            if (total > MAX_ARCHIVE_BYTES) throw DhavaBackupException("Backup is larger than 50 GB")
        }
        return total
    }
    ...
    private companion object {
        ...
        fun safeTarget(root: File, path: String): File {
            if (!allowedPath(path)) throw DhavaBackupException("Backup path is not allowed: $path")
            val target = root.resolve(path)
            val rootPath = root.canonicalFile.toPath()
            if (!target.canonicalFile.toPath().startsWith(rootPath)) {
                throw DhavaBackupException("Backup path escapes its workspace")
            }
            return target
        }
