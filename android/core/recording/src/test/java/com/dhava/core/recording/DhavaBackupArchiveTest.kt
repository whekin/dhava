...
class DhavaBackupArchiveTest {
    @Test fun `round trip preserves raw authored and imported inputs`() {
        val root = Files.createTempDirectory("dhava-backup-source").toFile()
        ...
        val archive = DhavaBackupArchive(root)
        ...
        val staging = Files.createTempDirectory("dhava-backup-target").toFile().resolve("staging")
    ...
    @Test fun `changed payload fails checksum before restore`() {
        val root = Files.createTempDirectory("dhava-backup-corrupt").toFile()
        ...
        val result = runCatching {
            DhavaBackupArchive(root).extract(
                ByteArrayInputStream(bytes),
                Files.createTempDirectory("dhava-backup-corrupt-target").toFile().resolve("staging"),
            )
        }
        assertTrue(result.exceptionOrNull() is DhavaBackupException)
    }

    @Test fun `non Dhava zip is rejected from its first entry`() {
        val bytes = ByteArrayOutputStream().also { output ->
        ...
        val result = runCatching {
            DhavaBackupArchive(Files.createTempDirectory("dhava-not-backup").toFile())
